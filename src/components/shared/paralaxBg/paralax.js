...
const ParallaxSection = ({ title, description, linkText, linkHref, videoSrc, titleStyle, descriptionStyle, linkStyle }) => {
  ...
  return (
    <div className="relative h-[500px] w-full">
      ...
      <div className="absolute top-0 left-0 w-full h-full flex flex-col gap-5 justify-center items-start p-2 px-2 pb-12 md:px-8 md:p-4 bg-black bg-opacity-50">
        <h3
          data-aos="fade-in"
          data-aos-easing="ease-in-out"
          data-aos-duration="1000"
          className={`text-2xl font-bold font-playfair text-left md:text-6xl text-white ${titleStyle}`}
        >
          {title}
        </h3>
        <p
